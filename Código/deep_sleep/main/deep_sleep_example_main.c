...
{   
    ...
    //Leer y configurar el sensor VELM7700
    //esp_err_t error = configurar_i2c();

    //ESP_ERROR_CHECK(error);

    luz[num_lecturas] = get_luz();
    HR[num_lecturas] = get_HR();
    temperatura[num_lecturas] = get_temp();
    printf("\n\n----------------- TimeStamp --------------------");
    ...
    if (num_lecturas == NUM_OF_DATA)
    {
        ...
        exito_thingsboard = enviar_datos(time_stamps, luz, temperatura, HR);
    ...
    printf("\nNumber of data measurements: %d\n", num_lecturas);
    
    //FIN Leer y configurar el sensor VELM7700

    //vTaskDelay(1000 / portTICK_PERIOD_MS);

    const int wakeup_time_sec = 60;  // 300 = 5 min
